export const StringsApp = {
  tituloCabecalho: 'SOLUÇÕES DE TECNOLOGIA PARA SUA EMPRESA',

  tituloSobre: 'Sobre a ABC',
  tituloNoticia: 'Notícias da mídia',
  resumoSobre:
    'A empresa de tecnologia ABC é uma líder inovadora no desenvolvimento de soluções tecnológicas avançadas. Especializada em metodologias ágeis, a ABC oferece serviços que abrangem desde a criação de softwares personalizados até a implementação de sistemas de gestão empresarial. ',

  tituloCardUmHome: 'Doações para o Rio grande do sul',
  tituloCardDoisHome: 'ABC fecha parceria com gigante do varejo',
  tituloCardTresHome: 'A evolução da inteligência artificial',
  resumoCardHome:
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed diam urna, euismod at porttitor in, dignissim eu mauris. In sem nibh, viverra eu risus sed,',
  tituloCabecalhoCliente: 'NÓS FAZEMOS A DIFERENÇA',
  subtituloCaseCliente: 'Case de sucesso',
  subtituloAlgunsClientes: 'Conheça alguns clientes',
  resumoClientes:
    'Conheça alguns clientes que transformaram seus negócios com as soluções inovadoras da empresa ABC.',
  tituloCardVajero: 'Case Varejo',
  tituloCardPet: 'Case setor Pet',
  tituloCardFinancas: 'Case Finanças',
  resumoDepoimento:
    'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis dolor nulla. Phasellus egestas purus ut quam malesuada, eu tristique tortor posuere. Phasellus massa arcu, rhoncus vitae maximus  vitae, lacinia a dui.',
  tituloCase: 'Case Petlove',
  resumoCasePrimeiroParagrafo:
    'A Petlove, renomada empresa do setor pet, recentemente passou por uma transformação digital impressionante, graças ao projeto liderado pela empresa ABC. Este case fictício ilustra como a adoção de uma metodologia ágil, aliada ao uso de tecnologia de ponta e um compromisso com a qualidade, pode revolucionar uma organização. A parceria com a ABC permitiu à Petlove modernizar seus processos internos e aprimorar a experiência do cliente de maneira significativa.',
  resumoCaseSegundoParagrafo:
    'Utilizando a metodologia ágil, a ABC estruturou o projeto de forma a permitir entregas rápidas e contínuas. Isso facilitou a adaptação e o ajuste das soluções de acordo com as necessidades específicas da Petlove, sem comprometer os prazos. As equipes multifuncionais trabalharam de maneira colaborativa, promovendo uma comunicação eficiente e garantindo que cada fase do projeto estivesse alinhada com os objetivos estratégicos da Petlove. A flexibilidade do método ágil permitiu que a Petlove respondesse rapidamente às mudanças do mercado e às expectativas dos clientes.',
  resumoCaseTerceiroParagrafo:
    ' A transformação digital também envolveu a implementação de tecnologias de ponta, como inteligência artificial, análise de big data e plataformas de e-commerce avançadas. A ABC assegurou que todas as soluções tecnológicas fossem integradas de forma harmoniosa, otimizando a operação da Petlove e proporcionando uma experiência de compra personalizada e eficiente para os clientes. A dedicação à qualidade foi evidente em cada etapa do projeto, resultando em sistemas robustos e confiáveis que não apenas atenderam, mas superaram as expectativas da Petlove. Como resultado, a Petlove não só consolidou sua liderança no mercado, mas também estabeleceu novos padrões de excelência no setor pet.',
  tituloCardDesign: 'Design Think para o setor pet',
  tituloCardVeterinaria: 'Inteligência artificial na Veterinária',
  tituloCardTecnoPet: 'Integrando tecnologia e bem-estar pet',
  tituloServicosOferecidos: 'Serviços oferecidos',
  tituloQuemSomos: 'SOMUS UMA EMPRESA DE TECNOLOHIA.',
  resumoDepoimentoDois:
    '"Contratar os serviços da empresa ABC foi uma das melhores decisões para nossa empresa. Eles implementaram soluções tecnológicas inovadoras que aumentaram nossa eficiência e produtividade. A equipe é extremamente profissional e dedicada, sempre pronta para resolver problemas e oferecer suporte. Recomendo a ABC para qualquer empresa que deseja crescer e se destacar no mercado."',
  resumoSobreQuemSomosParagrafoUm:
    'A empresa de tecnologia ABC é uma líder inovadora no desenvolvimento de soluções tecnológicas avançadas. Especializada em metodologias ágeis, a ABC oferece serviços que abrangem desde a criação de softwares personalizados até a implementação de sistemas de gestão empresarial. ',
  resumoSobreQuemSomosParagrafoDois:
    'Com um foco constante em eficiência, qualidade e satisfação do cliente, a ABC tem ajudado diversas empresas a melhorar sua produtividade e a alcançar seus objetivos estratégicos. Seu compromisso com a inovação e a excelência a torna uma parceira confiável para negócios de todos os setores.',
  resumoCardQuemSomos:
    'Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem.',
}
