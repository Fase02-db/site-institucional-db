...
public class DriverFactory {
    public static WebDriver createInstance(BrowserEnum browser) {
        try {
            ...
            String message = "Sessão dao criada, versão do driver nao suportada.";
            ExtentTestManager.getTest().log(Status.FAIL, message);
            throw new SessionNotCreatedException(message, e);
        } catch (WebDriverException e) {
            String message = "Nao foi possivel encontrar o binário do driver.";
            ExtentTestManager.getTest().log(Status.FAIL, message);
